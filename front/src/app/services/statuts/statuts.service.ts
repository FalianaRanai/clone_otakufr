...
export class StatutsService {
  ...
  constructor(private http: HttpClient) { }
    ...
    update(id: number, realisateur: any): Observable<any> {
      return this.http.put<any>(`${this.apiUrl}/statuts/${id}`, realisateur);
    }
  
    add(realisateur: any): Observable<any> {
      return this.http.post<any>(`${this.apiUrl}/statuts`, realisateur);
    }
  
    delete(id: number): Observable<any> {
      return this.http.delete<any>(`${this.apiUrl}/statuts/${id}`);
    }
  
    search(search:string, page = 1, sample = 10): Observable<any> {
      return this.http.get<any>(`${this.apiUrl}/statuts/search?page=${page}&sample=${sample}&search=${search}`);
    }
}
